...
4. Print a string to the console containing the letiable from step 3. (Something like "Is Mark's BMI higher than John's? true").
...
if (height || height === 0) {
    console.log('letiable is defined');
} else {
    console.log('letiable has NOT been defined');
}
...
6. Create a function (not a method) to calculate the average of a given array of tips. HINT: Loop over the array, and in each iteration store the current sum in a letiable (starting from 0). After you have the sum of the array, divide it by the number of elements in it (that's how you calculate the average)
